# newton's method for forward interpolation
...
# function to determine the value of y using forward interpolation
...
def forward(x,xp,yp):
    ...
    sum = y_diff[0][0] # declare a variable assigning the value of the first element of first row.
    
    u = (x - xp[0])/(xp[1]-xp[0]) # calculating the value of u

    for i in range(1,n):
        sum += (u_dec(u,i)*y_diff[0][i])/fac(i)  # applying the formula to sum-up everything together
